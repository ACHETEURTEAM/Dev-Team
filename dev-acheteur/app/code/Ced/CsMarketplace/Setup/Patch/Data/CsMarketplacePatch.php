...
use Ced\CsMarketplace\Setup\CsMarketplaceSetupFactory; // Make sure this is correct
use Ced\CsMarketplace\Model\Vendor\FormFactory; // Ensure you have the correct form factory
...
class CsMarketplacePatch implements DataPatchInterface
{
    ...
    private $csmarketplaceSetupFactory; // Ensure this is the correct type
    private $store;
    ...
    public function __construct(
        ...
        CsMarketplaceSetupFactory $csmarketplaceSetupFactory, // Corrected here
        ...
        $this->csmarketplaceSetupFactory = $csmarketplaceSetupFactory; // Ensure this is assigned correctly
...
protected function assignTheme()
    {
      ...
      // Get the current theme value from the configuration
        $themeValue = $this->scopeConfig->getValue(self::THEME_ID, ScopeInterface::SCOPE_STORE);

        // If no theme is set, assign the default theme
        if ($themeValue === null) {
            ...
            foreach ($themes as $theme) {
                if ($theme->getCode() === self::THEME_NAME) {
                    ...
                    $themeAssigned = true;
                    break; // Exit the loop once the theme is assigned
                }
            }

            if (!$themeAssigned) {
                // Log a warning that the expected theme was not found
