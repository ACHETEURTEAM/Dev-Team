...
class Interceptor extends \Ced\CsMarketplace\Model\Core\Store implements \Magento\Framework\Interception\InterceptorInterface
{
    ...
    public function __construct(\Magento\Framework\Module\Manager $moduleManager, \Ced\CsMarketplace\Model\VendorFactory $vendorFactory, \Magento\Framework\Registry $registry, \Magento\Framework\Model\Context $context, \Magento\Framework\Api\ExtensionAttributesFactory $extensionFactory, \Magento\Framework\Api\AttributeValueFactory $customAttributeFactory, \Magento\MediaStorage\Helper\File\Storage\Database $coreFileStorageDatabase, \Magento\Store\Model\ResourceModel\Store $resource, \Magento\Framework\App\Cache\Type\Config $configCacheType, \Magento\Framework\UrlInterface $url, \Magento\Config\Model\ResourceModel\Config\Data $configDataResource, \Magento\Framework\Filesystem $filesystem, \Magento\Framework\App\RequestInterface $request, \Magento\Framework\App\Config\ReinitableConfigInterface $config, \Magento\Store\Model\StoreManagerInterface $storeManager, \Magento\Framework\App\Http\Context $httpContext, \Magento\Framework\Session\SessionManagerInterface $session, \Magento\Framework\Session\SidResolverInterface $sidResolver, \Magento\Directory\Model\CurrencyFactory $currencyFactory, \Magento\Store\Model\Information $information, \Magento\Store\Api\GroupRepositoryInterface $groupRepository, \Magento\Store\Api\WebsiteRepositoryInterface $websiteRepository, $currencyInstalled, $isCustomEntryPoint = false, ?\Magento\Framework\Data\Collection\AbstractDb $resourceCollection = null, array $data = [])
    {
        $this->___init();
        parent::__construct($moduleManager, $vendorFactory, $registry, $context, $extensionFactory, $customAttributeFactory, $coreFileStorageDatabase, $resource, $configCacheType, $url, $configDataResource, $filesystem, $request, $config, $storeManager, $httpContext, $session, $sidResolver, $currencyFactory, $information, $groupRepository, $websiteRepository, $currencyInstalled, $isCustomEntryPoint, $resourceCollection, $data);
    }
    ...
    {
        $pluginInfo = $this->pluginList->getNext($this->subjectType, 'preparePath');
        return $pluginInfo ? $this->___callPlugins('preparePath', func_get_args(), $pluginInfo) : parent::preparePath($path, $group, $case);
    }
